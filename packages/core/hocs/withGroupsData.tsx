...
import {
  checkIos, getUniqUserId, initGA, writeToDebug,
} from '../utils';
...
const withGroupsData = (
  ...
  const handleOpenGroup = useCallback(
    (groupId: string) => {
      ...
      return API.statistics.group.onOpen({
        groupId,
        uniqUserId: userId,
        language,
      });
  ...
  const handleFinishQuiz = useCallback(
    ...
    [userId, language, getGroupCache, setGroupCache, getStoryCache, setStoryCache],
  );
  ...
  const handleCloseGroup = useCallback(
    (groupId: string) => {
      ...
      return API.statistics.group.onClose({
        groupId,
        uniqUserId: userId,
        language,
      });
    },
    [groupDuration, userId, language],
  );
  ...
  const handleOpenStory = useCallback(
    ...
    [data, userId, language, handleFinishQuiz],
  );
        ...
        API.stories
          ...
          .then((storiesData) => {
            ...
            if (!storiesData.data.error) {
              ...
              setGroupsWithStories((prevState) => prevState.map((item: any) => {
  ...
  useEffect(() => {
    if (loadStatus === 'loaded' && groupsWithStories.length && userId) {
      if (groupsWithStories.length) {
        const adaptedData = adaptGroupData(groupsWithStories, userId, language, isMobile);
  ...
  }, [loadStatus, groupsWithStories, userId, language, isMobile]);
